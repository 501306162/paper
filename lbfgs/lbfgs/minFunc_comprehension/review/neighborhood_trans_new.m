function ddk=neighborhood_trans_new(ind,original_image,K)
% return the neighbors'index of 'ind ' pixel 
N=numel(size(K));
switch (N)
    case 2
       ddk= neighborhood2d(ind,original_image,K);
    case 3
       ddk= neighborhood3d(ind,original_image,K);   
    otherwise
       fprintf('%d is not include in the neighborhood type!',N); 
end

end
function ddk=neighborhood2d(ind,image,K)
% return the neighbors'index of 'ind ' pixel
