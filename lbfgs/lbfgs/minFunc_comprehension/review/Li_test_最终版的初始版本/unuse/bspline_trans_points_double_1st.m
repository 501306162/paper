function Tlocal=bspline_trans_points_double_1st(O_trans,Spacing,X,check)
...
for r=0:1,
    ...
    Check_bound1(:,r+1)=(IndexO1l(:,r+1)<0)|(IndexO1l(:,r+1)>(size(O_trans,1)-1));  % 判断每个点是否超出图像大小
...
for l=0:1,
    for m=0:1,
        ...
        c=Ox(IndexO1(:)+IndexO2(:)*size(Ox,1)+1);  % i,j 号网格点...像素点四周网格点的x坐标
        Tlocalx=Tlocalx+Check_bound_inv(:).*ab.*c; % 获得每个像素点移动后的x坐标...即经过B样条处理的坐标
