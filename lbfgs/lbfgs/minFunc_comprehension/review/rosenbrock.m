...
if nargout > 1
  df = zeros(D, 1);
  df(1:D-1) = - 400*x(1:D-1).*(x(2:D)-x(1:D-1).^2) - 2*(1-x(1:D-1)); % 对xi求导...所以考虑范围应该是可以到达x_i+1的范围...才有导数值
  df(2:D) = df(2:D) + 200*(x(2:D)-x(1:D-1).^2);                      % 对x_i+1求导...所以考虑范围应该是可以到达x_i-1的范围...才有导数值
end
