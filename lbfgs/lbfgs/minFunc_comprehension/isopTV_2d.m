...
% w=2.5;
M_pyr=ceil(log2(min(m,n)/8));% 256--》5
M_ref=1;
M_iter=100;
M_lbgfs=5;
...
for j=1:(M_pyr+M_ref ) %
    if j<=M_pyr     %  金字塔创建 delta 大小
        ...
        f_mj=imresize(f_m,temp_size,'nearest');  %(Mpyr - j )-th Gaussian pyramid level of fm
        f_fj=imresize(f_f,temp_size,'nearest');  %(Mpyr - j )-th Gaussian pyramid level of ff
