function [Iout,Tx,Ty]=bspline_transform_2d_double(O_trans,Iin,dx,dy,mode)
...
Tlocal=bspline_trans_points_double_kx(O_trans,[dx dy],[x(:) y(:)],false);
...
if(nargout>1)
    ...
    Tx=reshape(Tlocal(:,1),[size(Iin,1) size(Iin,2)]);  
    Ty=reshape(Tlocal(:,2),[size(Iin,1) size(Iin,2)]); 
end
